...
class KingSafetyEvaluatorTest {
	...
	public void test_evaluateKingSafety_safe()throws IllegalNotationException {
		...
		assertEquals(0, SUT.evaluateKingSafety(SUT.bd.mae.calculateCountedAttacksAndMobility(SUT.bd.me), true)); // 5 squares, can be attacked by three pieces
	}
	...
	public void test_evaluateKingSafety_notVerySafe()throws IllegalNotationException {
		...
		assertEquals(0, SUT.evaluateKingSafety(SUT.bd.mae.calculateCountedAttacksAndMobility(SUT.bd.me), false)); // diagonals 7 squares, can be attacked by two pieces; r'n'f 9 squares can be attacked by three pieces
	}
	
	@Test
	public void test_evaluateKingSafety_No_inEndgame()throws IllegalNotationException {
		setUpPosition("8/8/8/8/8/8/8/K7 w - - 0 1");
		assertEquals(-0, SUT.evaluateKingSafety(SUT.bd.mae.calculateCountedAttacksAndMobility(SUT.bd.me), true));
	}
	...
	public void test_evaluateKingSafety_No_opposingBishopWrongColour()throws IllegalNotationException {
		...
		if (KingSafetyEvaluator.ENABLE_TWEAKED_KING_FLIGHT_SQUARES) {
			...
			assertEquals(-24, SUT.evaluateKingSafety(SUT.bd.mae.calculateCountedAttacksAndMobility(SUT.bd.me), true)); // 7*2*2 rnf 0 diag = 28
		}
	...
	public void test_evaluateKingSafety_Yes_opposingBishopRightColour()throws IllegalNotationException {
		...
		if (KingSafetyEvaluator.ENABLE_TWEAKED_KING_FLIGHT_SQUARES) {
			...
			assertEquals(-43, SUT.evaluateKingSafety(SUT.bd.mae.calculateCountedAttacksAndMobility(SUT.bd.me), true));
		}
	}
	...
	public void test_evaluateKingSafety_Yes_opposingQueenBishop()throws IllegalNotationException {
		...
		assertEquals(-75, SUT.evaluateKingSafety(SUT.bd.mae.calculateCountedAttacksAndMobility(SUT.bd.me), false));  // 1*2*2 diag = 4; 7*2*3 = 42 r'n'f; 4+42 = 46 
			...
			assertEquals(-20, SUT.evaluateKingSafety(SUT.bd.mae.calculateCountedAttacksAndMobility(SUT.bd.me), true));  // (5 up right + 2 up left) *2 *1bish = 14; (7 up + 2 left + 5 right) * 2 *2rooks = 28*2; 56+14 = 70
		}
	}
	...
	public void test_evaluateKingSafety_TwoKnights_attackBlack()throws IllegalNotationException {
		...
		assertEquals(0, SUT.evaluateKingSafety(SUT.bd.mae.calculateCountedAttacksAndMobility(SUT.bd.me), false)); // One knight attacks the black king zone
	}

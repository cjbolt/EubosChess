...
public class MoveListTest {
	...
	public void testCreateMoveList_CapturesFirstThenChecks() throws InvalidPieceException, IllegalNotationException {
		...
		assertEquals(new GenericMove("h7f5"), Move.toGenericMove(it.nextInt()));
	}
	...
	public void testCreateMoveList_ChecksFirstThenCastles() throws InvalidPieceException, IllegalNotationException {
		...
		PrimitiveIterator.OfInt it = classUnderTest.iterator();
		assertEquals(new GenericMove("h7f5"), Move.toGenericMove(it.nextInt()));
		assertEquals(new GenericMove("e1g1"), Move.toGenericMove(it.nextInt()));
	}
	...
	public void test_setBestMove() throws IllegalNotationException {
		...
		PrimitiveIterator.OfInt it = classUnderTest.iterator();
		assertFalse(Move.areEqual(expected, it.nextInt()));
		classUnderTest.reorderWithNewBestMove(expected);
		it = classUnderTest.iterator();
		assertTrue(Move.areEqual(expected, it.nextInt()));
	}
	...
	public void test_bestMove_ChangedAgain() throws IllegalNotationException {
		...
		PrimitiveIterator.OfInt it = classUnderTest.iterator();
		assertNotEquals(expected, Move.toGenericMove(it.nextInt()));
		
		classUnderTest.reorderWithNewBestMove(Move.toMove(new GenericMove("a7a6")));
		it = classUnderTest.iterator();
		assertEquals(new GenericMove("a7a6"), Move.toGenericMove(it.nextInt()));
		
		classUnderTest.reorderWithNewBestMove(Move.toMove(expected));
		it = classUnderTest.iterator();
		assertEquals(expected, Move.toGenericMove(it.nextInt()));
	}
	...
	public void test_whenChangedBestCapture_BothIteratorsAreUpdated() throws IllegalNotationException {
		...
		GenericMove first = Move.toGenericMove(it.nextInt());
		...
		assertTrue(Move.areEqual(newBestCapture, it.nextInt()));
		
		it = classUnderTest.getIterator(EXTENDED);
		assertTrue(it.hasNext());
		assertTrue(Move.areEqual(newBestCapture, it.nextInt()));
	}
	...
	public void test_whenCheckAndCapturePossible() throws IllegalNotationException {
		...
		PrimitiveIterator.OfInt it = classUnderTest.iterator();
		assertEquals(new GenericMove("e4g2"), Move.toGenericMove(it.nextInt())); // Check and capture
		assertEquals(new GenericMove("g4g2"), Move.toGenericMove(it.nextInt())); // capture
		assertEquals(new GenericMove("g4h4"), Move.toGenericMove(it.nextInt())); // check
	}

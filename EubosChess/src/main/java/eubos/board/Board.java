...
//			}
	...
	public int getQueenPosition(boolean isWhite) {
		if (EubosEngineMain.ENABLE_PIECE_LISTS) {
			return pieceLists.getQueenPos(isWhite);
		} else {
    ...
    */
	...
	public void whiteEndgameExcept(IAddMoves ml, long except_mask) {
		long side = whitePieces;
		long scratchBitBoard = pieces[Piece.KING] & side;
		scratchBitBoard &= except_mask;
		int bit_offset = BitBoard.INVALID;
		if (scratchBitBoard != 0L && (bit_offset = BitBoard.convertToBitOffset(scratchBitBoard)) != BitBoard.INVALID) {			
			Piece.king_generateMoves_White(ml, this, bit_offset);
		}
	...
	public void blackEndgameExcept(IAddMoves ml, long except_mask) {
		long side = blackPieces;
		long scratchBitBoard = pieces[Piece.KING] & side;
		scratchBitBoard &= except_mask;
		int bit_offset = BitBoard.INVALID;
	...
	public void whiteMidgameExcept(IAddMoves ml, long except_mask) {
		long side = whitePieces;
		long scratchBitBoard = pieces[Piece.BISHOP] & side;
		scratchBitBoard &= except_mask;
		int bit_offset = BitBoard.INVALID;
	...
	public void blackMidgameExcept(IAddMoves ml, long except_mask) {
		long side = blackPieces;
		long scratchBitBoard = pieces[Piece.BISHOP] & side;
		scratchBitBoard &= except_mask;
		int bit_offset = BitBoard.INVALID;
	...
	public void getRegularPieceMovesExceptingOnePiece(IAddMoves ml, boolean ownSideIsWhite, int except_piece) {
		long bit_mask = ~(1L << except_piece);
		if (me.isEndgame()) {
			if (ownSideIsWhite) {
				whiteEndgameExcept(ml, bit_mask);
			} else {
				blackEndgameExcept(ml, bit_mask);
			}
		} else {
			if (ownSideIsWhite) {
				whiteMidgameExcept(ml, bit_mask);
			} else {
				blackMidgameExcept(ml, bit_mask);
			}
		}
	}
	...
	public boolean hasKnightCheckForkThreat(boolean onMoveIsWhite) {
		boolean fork = false;
		int kingBitOffset = this.getKingPosition(onMoveIsWhite);
		long enemy = onMoveIsWhite ? blackPieces : whitePieces;
		long own = allPieces & ~enemy;
		long enemyKnights = pieces[Piece.KNIGHT] & enemy;
		long potentialKnightChecks = enemyKnights & SquareAttackEvaluator.KnightForks_Lut[kingBitOffset];
		if (potentialKnightChecks != 0L) {
			long own_rook_queens = (pieces[Piece.ROOK] | pieces[Piece.QUEEN]) & own;
			int bit_offset = 0;
			while (enemyKnights != 0L && (bit_offset = BitBoard.convertToBitOffset(enemyKnights)) != BitBoard.INVALID) {
				long knightForksFromKnight = SquareAttackEvaluator.KnightForks_Lut[bit_offset];
				long enemyKnightMask = (1L << bit_offset);
				if ((knightForksFromKnight & own_rook_queens) != 0L &&
					(enemyKnightMask & SquareAttackEvaluator.KnightForks_Lut[kingBitOffset]) != 0L) {
					fork = true;
				}
				enemyKnights ^= enemyKnightMask;
			}
		}
		return fork;
	}

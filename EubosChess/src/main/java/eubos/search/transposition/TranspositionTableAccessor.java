...
public class TranspositionTableAccessor implements ITranspositionAccessor {
	...
	public long setTransposition(long trans, byte new_Depth, short new_score, byte new_bound, int new_bestMove) {
		if (EubosEngineMain.ENABLE_TRANSPOSITION_TABLE) {
			...
			if (trans == 0) {
				...
				trans = hashMap.getTransposition(pos.getHash());
				if (trans == 0L) {
					trans = createTranpositionAddToTable(new_Depth, new_score, new_bound, new_bestMove);
					is_created = true;
				}
			}
			if (!is_created) {
				long old_trans = trans;
				trans = Transposition.checkUpdate(trans, new_Depth, new_score, new_bound, new_bestMove);
				if (old_trans != trans) {
	...
	private long createTranpositionAddToTable(byte new_Depth, short new_score, byte new_bound, int new_bestMove) {
		if (SearchDebugAgent.DEBUG_ENABLED) sda.printCreateTrans(pos.getHash());
		long new_trans = Transposition.valueOf(new_Depth, new_score, new_bound, new_bestMove);
